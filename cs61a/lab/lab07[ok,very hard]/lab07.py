...
def trade(first, second):
    ...
    while m<=len(first) and n<=len(second): #不能写or，否则在有的例子里会导致一方疯涨
...
def make_glookup(class_assignments):
    ...
    def calculate_score(key,value):
        ...
        stu_total=0
        for key in score:
            total=total+cs61a[key]
            stu_total=stu_total+score[key]
        return stu_total/total
