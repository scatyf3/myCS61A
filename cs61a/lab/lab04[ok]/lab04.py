...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    def make_city(name,x,y):
        city=[name,x,y]
        return city
    distance=sqrt((city_a[1]-city_b[1])**2+(city_a[2]-city_b[2])**2)
    return distance
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_tmp=make_city("city_tmp",lat,lon)
    dis_a=distance(city_a,city_tmp)
    dis_b=distance(city_b,city_tmp)
    if dis_a>dis_b:
        return city_a[0]
    else:
        return city_b[0]
