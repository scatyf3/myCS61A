...
class Player:
    ...
    def draw(self):
        """Draw a card from the player's deck and add it to their hand.从玩家的牌组中抽牌，将之加入手牌
        >>> test_card = Card('test', 100, 100)
        >>> test_deck = Deck([test_card.copy() for _ in range(6)])
        >>> test_player = Player(test_deck, 'tester')
        >>> test_player.draw()
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        if len(opponent.hand)>=3 and len(opponent.deck.cards)>=3:
            opponent.hand=opponent.hand[3:]
            for _ in range(3):
                opponent.hand.append(opponent.deck.cards.pop())
