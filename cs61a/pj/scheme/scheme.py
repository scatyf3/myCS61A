...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(operator)#是raise error用而不是返回tf用
        operands=rest.map(lambda x:scheme_eval(x,env))#不知道剩下的有多少，只能用map，并且这是pair自带的一个操作
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        "*** YOUR CODE HERE ***"
        is_found=0
        while self is not  None and is_found==0:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #这里仅仅评估单个数字，所以可以直接rest.first。并且rest是pair形式，eval默认其first是operator
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    print('DEBUG:2',expressions.first)
    return expressions.first
    # END PROBLEM 6
